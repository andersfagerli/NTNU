...
class BasicModel(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.feature_map_zero = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Conv2d(num_filters*2, self.output_channels[0], 3, 2, 1)
        )

        self.feature_map_one = torch.nn.Sequential(
            torch.nn.ReLU(),
            torch.nn.Conv2d(self.output_channels[0], 128, 3, 1, 1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, self.output_channels[1], 3, 2, 1),
        )

        self.feature_map_two = torch.nn.Sequential(
            torch.nn.ReLU(),
            torch.nn.Conv2d(self.output_channels[1], 256, 3, 1, 1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(256, self.output_channels[2], 3, 2, 1),
        )

        self.feature_map_three = torch.nn.Sequential(
            torch.nn.ReLU(),
            torch.nn.Conv2d(self.output_channels[2], 128, 3, 1, 1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, self.output_channels[3], 3, 2, 1),
        )

        self.feature_map_four = torch.nn.Sequential(
            torch.nn.ReLU(),
            torch.nn.Conv2d(self.output_channels[3], 128, 3, 1, 1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, self.output_channels[4], 3, 2, 1),
        )

        self.feature_map_five = torch.nn.Sequential(
            torch.nn.ReLU(),
            torch.nn.Conv2d(self.output_channels[4], 128, 3, 1, 1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, self.output_channels[5], 3, 1, 0)
        )
