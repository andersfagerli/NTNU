...
%             end
        ...
        function zpred = h(obj, eta)
            ...
            % polar
            zpred = [ sqrt(z_b(1, :).^2 + z_b(2, :).^2); 
                      atan2(z_b(2, :), z_b(1, :))     ];
            
            % make column great again
            zpred = zpred(:); 
        end
...
%             end
        ...
        function [etaadded, Padded] = addLandmarks(obj, eta, P, z)
            ...
            for j = 1:numLmk
                ...
                lmnew(inds) = zj(1)*rot(:,1) + eta(1:2) + Rot * obj.sensOffset; % mean
                Gx(inds, :) = [eye(2), zj(1) * [-sin(zj(2) + eta(3)); cos(zj(2) + eta(3))] + rotpi * obj.sensOffset]; % jac h^1 wrt. x
