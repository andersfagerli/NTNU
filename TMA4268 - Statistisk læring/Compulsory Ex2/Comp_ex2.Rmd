...
$$
\hat{Y_i}=\beta_0+\hat\beta_{logcarat}\cdot\text{logcarat}_i+\hat\beta_{cut}\cdot\text{cut}_i+\hat\beta_{color}\cdot\text{color}_i+\hat\beta_{clarity}\cdot\text{clarity}_i+\hat\beta_{xx}\cdot\text{xx}_i
$$
...
Using best subset selection, we may produce the combination of covariates that gives the best score according to some criteria, here the AIC. The resulting model consists of `npreg`, `glu`, `bmi`, `ped` and `age`.
...
Extending to support vector *machines*, we choose a non-linear decision boundry. This boundry is set by choosing a specific *kernel*, $K(\boldsymbol{x},\boldsymbol{x}_i)$, giving e.g a radial or polynomial boundry. The resulting classifier is $f({\bf x}_i)=\beta_0 +\sum_{l\in \cal{S}} \alpha_l K({\bf x}_i,{\bf x}_l)$.
...
# train_data <- scale(ctrain[,-1], center = mean, scale = std)
# test_data <- scale(ctest[,-1], center = mean, scale = std)
...
diabetes = unlist(ctrain$diabetes)
...
for (j in 1:k) {
    ...
    for (i in 1:length(grid)) {
        ...
        fit = nnet(as.factor(diabetes) ~ ., data = new, size = thissize, linout = FALSE,
            maxit = 5000)
        pred = predict(fit, newdata = new[thisvalid, ], type = "raw")
        resmat[i, j] = sum((pred[, 1]==diabetes[thisvalid]))
    }
}

#misclass = 1- table(resmat, truth = ctest[, 1])
misclass = 1-apply(resmat, 1, sum)/nrow(train_data)
plot(grid, misclass, type = "l")
misclass
```
We find the lowest misclassification rate when we use 25 nodes, and use this when fitting the model with the full test set.
```{r,eval = TRUE}
fitnnet = nnet(as.factor(diabetes) ~., size = 25, data = train_data, lineout = FALSE, maxit = 1000)
summary(fitnnet)
```
In nnet we can only have one hidden layer, and for single hidden layer feeedforwad networks we have the model:
$$\hat{y}_c({\bf x})=\phi_o(\beta_{0c}+\sum_{m=1}^{M}\beta _{mc}z_m)=\phi_o(\beta_ {0c}+\sum_{m=1}^{M}\beta_{mc}\phi_h(\alpha_{0m} +\sum_{j=1}^{p}\alpha_{jm}x_j) $$ where $\phi_0(x)=x$.
...
We estimate det unknown parameters $\theta$ by minimizing the loss function $J(\theta)$.
COMMENT: Are we using gradient descent to achieve this?
Our tuning parameters are: $\alpha:{jm}$ an $\beta_{mc}$
When using neural networks we don't make any assumptions about the data, errors or targets.
...
```
We make a confusion matrix and calculate the accuracy:
```{r,eval = TRUE}
#accuracy
conf_nnet = table(prednnet, truth = ctest[, 1])
accuracy_nnet= sum(diag(conf_nnet))/(sum(conf_nnet))
accuracy_nnet
```
The ROC and AUC are given below
```{r,eval = TRUE}
#ROC and AUC
nnetroc = roc(test_data[,1], as.numeric(prednnet))
auc(nnetroc)
plot(nnetroc)
```

KAN VURDERE Å SE PÅ: We can add weight decays = penalties in the nnet.
