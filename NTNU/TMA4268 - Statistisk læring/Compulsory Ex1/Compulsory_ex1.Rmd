...
yhat.train = sapply(ks, function(k) {
  class::knn(train = M[tr,-1], cl = M[tr,1],test = M[-1], k = k) #Oppgi test = M[tr,-1], altså test-set = training-set?
})
train.equal = I(yhat.train == M[,1])
train.e = apply(train.equal == FALSE, 2, sum) #Feil å bruke "== FALSE"?
train.e = train.e/nrow(train.equal)

yhat.test = sapply(ks, function(k) {
  class::knn(train = M[-tr,-1], cl = M[-tr,1],test = M[-1], k = k) #Riktig måte for test-set??
})
test.equal = I(yhat.test == M[,1])
