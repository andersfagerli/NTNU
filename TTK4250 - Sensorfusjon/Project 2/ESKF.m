classdef ESKF
    ...
    methods
        ...
        function xnompred = predictNominal(obj, xnom, acc, omega, Ts)
            ...
            % predictions
            posPred = pos + vel*Ts + 0.5*(R*acc+obj.g)*Ts^2;
            ...
            quatPred = normalize(quatPred, 'norm', 2);
        ...
        function [Ad, GQGd] = discreteErrMats(obj, xnom, acc, omega, Ts)
            ...
            % use Van Loan
            V = [       -A, G*obj.Qerr*G';
                 zeros(15),            A']*Ts; %; % the matrix exponent in Van Loan
            VanLoanMat = expm(V); % can potentially be slow
             
            % exctract relevat matrices.
            V1 = VanLoanMat(16:30, 16:30); 
            Ad = V1';
            V2 = VanLoanMat(1:15, 16:30);
            GQGd = V1' * V2;
        end
        ...
        function [xinjected, Pinjected] = inject(~, xnom, deltaX, P)
            ...
            % Inject error state into nominal state (quaternions cannot be added), using eq. (10.72)
            xinjected(1:3) = xnom(1:3) + deltaX(1:3);
            xinjected(4:6) = xnom(4:6) + deltaX(4:6);
            
            xinjected(7:10) = quatProd(xnom(7:10), [1; 0.5*deltaX(7:9)]);
            
            xinjected(11:13) = xnom(11:13) + deltaX(10:12);
            xinjected(14:16) = xnom(14:16) + deltaX(13:15);
            % make sure quaterion is normalized
            xinjected(7:10) = normalize(xinjected(7:10), 'norm', 2);
                
            % compensate for injection in the covariance, using eq. (10.86)
            Ginject = blkdiag(eye(6), eye(3)-crossProdMat(0.5*deltaX(7:9)), eye(6));
        ...
        function [v, S] = innovationGNSS(~, xnom, P, zGNSSpos, RGNSS, leverarm)
            ...
            Hx = [eye(3), zeros(3,13)]; % using eq. (10.80)
            
            Q_dtheta = 0.5 * [-xnom(8:10)'; crossProdMat(xnom(8:10))+eye(3)*xnom(7)];
            X_dx = blkdiag(eye(6), Q_dtheta, eye(6));
            H = Hx * X_dx; % using eq. (10.78)
        ...
        function deltaX = deltaX(~, xnom, xtrue)
           ...
           qConj = [xnom(7); -xnom(8:10)]; % conjugated nominal quaternion, eq. (10.24)
