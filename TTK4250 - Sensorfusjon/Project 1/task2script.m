...
for k = 1:K
    ...
    NEES(k) = (xhat(:,k)-Xgt(:,k))' / Phat(:,:,k) * (xhat(:,k)-Xgt(:,k)); % see p.63 in book
...
% sensor 
r = 5; % 5
lambda = 7.5918e-6; % 7.5918e-6
PD = 0.95; % 0.95
gateSize = 5^2; % 5^2

% dynamic models
qCV = 0.0025; % 0.0025
qCT = [0.005, 0.0005]; % [0.005, 0.0005]
x0 = [6 25 5 0 0]'; % [6 25 5 0 0]'
P0 = diag([0.5, 0.5, 0.5, 0.5, 0.0005].^2); % diag([0.5, 0.5, 0.5, 0.5, 0.0005].^2)
...
for k=1:100
    ...
    NEES(k) = (xest(:,k)-Xgt(:,k))' / Pest(:,:,k) * (xest(:,k)-Xgt(:,k)); % see p.63 in book
...
inCI = sum((NEES >= ciNEES(1)) .* (NEES <= ciNEES(2)))/K * 100;
plot([1,K], repmat(ciNEES',[1,2])','r--')
text(104, -3, sprintf('%.2f%% inside CI', inCI),'Rotation',90);
