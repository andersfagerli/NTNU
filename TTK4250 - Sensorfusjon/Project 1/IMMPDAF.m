    ...
    methods
        ...
        function ll = loglikelihoodRatios(obj, Z, sprobs, x, P)
            ...
            for j = 1:m
                ...
                ll(j + 1) = llCond(j) + logPD; %... association loglikelihood ratio for detection j
            end
        end
        ...
        function [sprobsred, xred, Pred] = reduceMixture(obj, beta, sprobs, x, P)
            ...
            joint = sprobs .* repmat(beta', M, 1); %.. Joint probability for mode and association (M x m + 1)
            sprobsred = sum(joint, 2); %... marginal mode probabilities (M x 1)
            betaCondS = joint ./ repmat(sprobsred, 1, length(beta)); %... association probabilites conditionend on the mode probabilites (M x m + 1)
